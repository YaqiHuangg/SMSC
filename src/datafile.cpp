...
MCSFile::MCSFile(char* filename)
{
	...
	// eat the first line
	char line[MAX_LINE];
	mm.getline(line, MAX_LINE);
}

MCSFile::~MCSFile()
{
	if(mm.is_open()) mm.close();
}
...
coregraph* MCSFile::getNextGraph()
{
	if(!mm.good()) return NULL;

	coregraph* mcs = parseMCS();
	return mcs;
}

coregraph* MCSFile::parseMCS()
{
	char line[MAX_LINE];
