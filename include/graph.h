...
public:
    unsigned* vfreq;//保存一个图中节点标签出现的次数
    unsigned* efreq;//保存一个图中边标签（即elabel(i, j)）出现的次数

    unsigned** vertex_efreq;//保存节点i中与其相连边的标签（第二个下标）的个数，最后一项（elabel_map.size()）保存与节点i相连的边的总个数，即度数
...
private:
	vector<unsigned>* vlabel_index; // pointer to vlabel_index_mem, 保存节点标签出现的位置，即序号, id
